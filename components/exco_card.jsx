...
const ExcoCard = ({ exco, wrapperClassName, isActiveIndex }) => {
	const [isActive, setIsActive] = React.useState(isActiveIndex);
	React.useEffect(() => {
		setIsActive(isActiveIndex);
	}, [isActiveIndex]);

	const handleToggleIsActiveIndex = () => setIsActive(!isActive);

	return (
		<div className='xss:px-2 xss:py-4 sm:p-4 w-full max-w-[400px] max-h-[400px] h-[350px]'>
			<div
				onClick={handleToggleIsActiveIndex}
