...
const RawReqController = {
	...
	editAdmin: async (req, res) => {
		try {
			...
			const { adminUrl, firstname, secondname, lastname, username, email, password, gender, mobile, member_role, sameAsLoggedInUser } = req.body;
			...
			// ** RECORD IN ACTIVITY_LOG DATABASE
			await activityLog({
				deed: sameAsLoggedInUser ? ACTIVITY_TYPES.UPDATED_MY_PROFILE.title : ACTIVITY_TYPES.UPDATED_ADMIN_PROFILE.title,
				details: sameAsLoggedInUser
					? ACTIVITY_TYPES.UPDATED_MY_PROFILE.desc
					: `${ACTIVITY_TYPES.UPDATED_ADMIN_PROFILE.desc} - ${lastname} ${firstname}`,
				user_id: req?.user?._id,
				res,
			});
